...
info_punto <- info_punto %>%
  add_row(
    ...
    habitat = glue(params$puntos$HabitatGeneral,params$puntos$HabitatTipo))

# 3. Trasponer y dar nombre a las filas
info_puntot <- as.data.frame(t(info_punto))
row.names(info_puntot) <- c("Punto de muestreo","Voluntario/a","Temporada","Localización", "Hábitat general y tipo*")
...
\* Ver [ficha de hábitats de SEO/BirdLife](https://atlasaves.seo.org/wp-content/uploads/2021/12/Anexo-3-ficha-habitat-atlas-rep-2014_17.pdf)
...
info_grab <- info_grab %>%
  add_row(
    ...
    tarjetas = params$puntos$Tarjeta.Marca,
    pilas = params$puntos$Baterias
  )

info_grabt <- as.data.frame(t(info_grab))
row.names(info_grabt) <- c("ID de la grabadora","Configuración","Periodo de grabación en el punto","Minutos grabados","Sesiones con datos incompletos", "Archivos generados","Tarjeta de memoria", "Baterías")
